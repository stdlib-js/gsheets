...
/**
* Wraps a quinary function which accepts five numbers and returns a number.
*
* @private
* @param {Function} fcn - quinary function
...
function wrap( fcn, opts ) {
	...
	*/
	function wrapper( x1, x2, x3, x4 ) {
		...
		if ( !isNumber( x5 ) ) {
			if ( opts.nonnumeric === void 0 ) {
				throw new TypeError( format( 'invalid argument. Fifth argument must be a number or a range of numbers. Value: %s.', String( x5 ) ) );
			}
			return opts.nonnumeric;
		}
		v = fcn( x1, x2, x3, x4, x5 );
		if ( v !== v ) {
			return opts.nan;
		}
		if ( v === PINF ) {
